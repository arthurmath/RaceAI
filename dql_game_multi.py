...
class Car:
    ...
    def update(self, actions: list[int]):
        if not isinstance(actions, list): #on force la conversion en une liste
            actions = [actions]
        moved = False
...
class Session:        
    ...
    #def generate_cars(self): #génère les voitures possibilité de mettre la ligne dans reset
     #   self.car_list = [Car(self) for _ in range(self.nb_cars)]



    def reset(self, episode=1): #pour reset l'environnement
        self.episode = episode
        self.episode_done = False
        self.rewards = [0] * self.nb_cars
        self.prev_rewards = [0] * self.nb_cars
        self.terminateds = [False] * self.nb_cars
        self.car_list = [Car(self) for _ in range(self.nb_cars)] #création N voitures
    ...
    def draw(self):
        ...
        # Draw cars
        for car in self.car_list: #OK Pour plusieurs voitures
            if car.alive:
                car.draw(self)    
    ...
    def step(self, actions): #fait progresser l'environnement d'un pas de temps
        ...
        #actions = [actions]
        #print(actions)
        #for idx, car in enumerate(self.car_list): #applique les actions a chaque pas de temps
        if actions is None or len(actions) != len(self.car_list):
            raise ValueError(
                f"Mismatch: len(actions)={len(actions or [])} but len(car_list)={len(self.car_list)}. "
                f"Each car needs a corresponding action."
            )


        for idx, car in enumerate(self.car_list):
            if car.alive and actions[idx] is not None:
                car.update(actions[idx])
        
        # Check if all cars are dead
        self.nb_alive = sum([car.alive for car in self.car_list]) #nb de voitures vivantes
        if self.nb_alive == 0:
            self.episode_done = True
        ...
        self.states = self.get_states() #on récupère l'état courant
        step_rewards = self.get_rewards()
        
        return self.states, step_rewards, self.terminateds #il y avait des [0]

    
    def get_states(self): #pour chaque voiture, on a un vecteur d'état
        states = []
